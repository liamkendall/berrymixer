...
#Remove Tasmania bumblebee data
berry_single_updated <- berry_single_updated[!berry_single_updated$Species%in%"BR",]%>%
                        droplevels()

table(berry_single_updated$SPEC.COM)
...
m1 <- glmmTMB(FS~Species+SPEC.COM+Visitor1+sumvisits+(1|Block)+(1|Year),
              family="binomial",
              data = berry_single_updated2)
summary(m1)

m2 <- glm(FS~Species+SPEC.COM+Visitor1+sumvisits,
              family="binomial",
              data = berry_single_updated2)
par(mfrow=(c(2,2)))
summary(m2)
...
m2 <- glmmTMB(Fresh.wgt~SPEC.COM*Visitor1+sumvisits+(1|Block)+(1|Year),
              family="gaussian",
